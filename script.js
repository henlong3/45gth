...
let gattahuCurrentLevel = 15 * 3; // 초기 레벨 2배 -> 3배
let gattahuCurrentStage = 1;
let gattahuCurrentBetUnit = 0;
let gattahuTotalUnits = 15 * 3; // 시작 유닛 2배 -> 3배
...
const gattahuLevelMap = {
    ...
    15: { // 15
        1: { bet1: 6, bet2: 12, win: { type: 'calcLevelSumCurrentLevel', units: [6, 12] }, lose: { type: 'goto', stage: 2 } },
        2: { bet: 9, win: { type: 'goto', level: 18 }, lose: { type: 'gameOver' } }
    },
    18: { // 18
        1: { bet1: 6, bet2: 12, win: { type: 'calcLevelSumCurrentLevel', units: [6, 12] }, lose: { type: 'goto', stage: 2 } },
    ...
    24: { // 24
        1: { bet1: 6, bet2: 12, win: { type: 'calcLevelSumCurrentLevel', units: [6, 12] }, lose: { type: 'goto', stage: 2 } },
        2: { bet: 18, win: { type: 'goto', level: 36 }, lose: { type: 'gameOver' } }
    },
    27: { // 27
        1: { bet1: 6, bet2: 12, win: { type: 'calcLevelSumCurrentLevel', units: [6, 12] }, lose: { type: 'goto', stage: 2 } },
        2: { bet: 21, win: { type: 'goto', level: 42 }, lose: { type: 'gameOver' } }
    },
    30: { // 10 * 3
    ...
    81: { // 27 * 3
        ...
        4: { bet: 9 * 3, win: { type: 'goto', level: 30 * 3 }, lose: { type: 'gotoLevel', level: 12 * 3 } } // 단일 베팅
    },
    84: { // 28 * 3
        ...
        4: { bet: 8 * 3, win: { type: 'goto', level: 30 * 3 }, lose: { type: 'gotoLevel', level: 14 * 3 } } // 단일 베팅
    },
    87: { // 29 * 3
        1: { bet: 1 * 3, win: { type: 'goto', level: 30 * 3 }, lose: { type: 'goto', stage: 2 } }, // 단일 베팅
        2: { bet: 2 * 3, win: { type: 'goto', level: 30 * 3 }, lose: { type: 'goto', stage: 3 } }, // 단일 베팅
        3: { bet1: 3 * 3, bet2: 1 * 3, win: { type: 'calcLevelSumCurrentLevelSubtract', units: [3 * 3, 1 * 3], subtract: 3 * 3 }, lose: { type: 'goto', stage: 4 } },
        4: { bet: 7 * 3, win: { type: 'goto', level: 30 * 3 }, lose: { type: 'gotoLevel', level: 16 * 3 } } // 단일 베팅
    },
    90: { // 30 * 3
        1: { bet: 10 * 3, win: { type: 'gameWin' }, lose: { type: 'gameOver' } } // 예시: 레벨 30의 최종 베팅
    }
};
...
function gattahuInitializeGame() {
    gattahuCurrentLevel = 15 * 3; // 시작 레벨 2배 -> 3배
    gattahuCurrentStage = 1;
    gattahuTotalUnits = 15 * 3; // 시작 유닛 2배 -> 3배
...
function gattahuUpdateDisplay() {
    ...
    // 게임 승리 조건 확인
    if (gattahuTotalUnits >= 30 * 3) { // 승리 유닛 2배 -> 3배
        gattahuGameWin("축하합니다! 총 유닛이 90에 도달하여 게임에 승리했습니다!");
        return; // 승리 시 추가 로직 실행 방지
    }
    // 게임 패배 조건 확인 (초기화 시점 제외)
    // totalUnits가 0 이하가 되면 게임 오버. 단, 초기 totalUnits가 90일 때는 예외.
    if (gattahuTotalUnits <= 0 && !(gattahuCurrentLevel === 30 * 3 && gattahuCurrentStage === 1 && gattahuTotalUnits === 30 * 3)) { // 초기 유닛도 2배로 고려 -> 3배로
...
function gattahuHandleWin() {
    ...
    if (stageData.win.type === 'goto') {
        ...
        if (gattahuWinStreak === 2) { // 2연승
            ...
            // stageData.bet2는 이미 3배로 조정되었으므로 그대로 사용
...
function gattahuHandleLose() {
    ...
    // 레벨 66-78 (기존 22-26)의 4단계 특수 패배 로직
        ...
        if (unitsLost === (stageData.bet1)) { // bet1도 3배로 계산된 값과 비교
            nextLevel = gattahuCurrentLevel - (15 * 3); // 15 * 3 레벨 감소
            gattahuMessageEl.textContent = `패배! (1번째 베팅 실패) 레벨 ${nextLevel}로 이동합니다.`;
        } else if (unitsLost === (stageData.bet2)) { // bet2도 3배로 계산된 값과 비교
            if (stageData.lose.type === 'specialLoseLevel25_4') { // 기존 25레벨
                nextLevel = gattahuCurrentLevel + (1 * 3); // 1 * 3 레벨 증가
                gattahuMessageEl.textContent = `패배! (2번째 베팅 실패) 레벨 ${nextLevel}로 이동합니다.`;
            } else if (stageData.lose.type === 'specialLoseLevel26_4') { // 기존 26레벨
                nextLevel = gattahuCurrentLevel + (2 * 3); // 2 * 3 레벨 증가
                gattahuMessageEl.textContent = `패배! (2번째 베팅 실패) 레벨 ${nextLevel}로 이동합니다.`;
            } else { // 22, 23, 24의 4단계 2번째 베팅 패배 시 (2배된 레벨 기준 66, 69, 72) -> 3배로
